...
## スキル→ものさし翻訳 (ものさし化) ポリシー

- `/yt-import-skill ... ものさし化` は**検証済み (育った) スキルだけ**を採点基準の主成分にする。翻訳前の成熟度確認 (6a) を省略する変更は不可 — 未検証スキルの翻訳はプロファイル反映 (らしさの減点根拠) 止まり
- 翻訳は**字面ではなく意図** (「なぜそのルールがあるか」を帯に書き、字面は「既定の証拠」に格納)。字面準拠だけの軸は立てない
- 生成する軸は 5-7 本で、**動画固有軸 2 本 (ブリーフの約束回収・具体例の鮮度 相当) を必ず含める** — generator が同じスキルを使う構成ではスキル準拠軸の勾配が消えるため。この 2 本を削る変更は不可 (ユーザー明示要求時は開示付きで従う)
