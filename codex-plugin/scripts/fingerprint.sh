...
#
# 指紋の対象 = threshold / criteria / generator_skill / evaluator_skill / evaluator_runtime
#            / judges / host_vendor / judge_models / max_iterations
...
HASH=$(
  {
    jq -r '[.threshold, .criteria, .generator_skill, .evaluator_skill, (.evaluator_runtime // "skill"), (.judges // "host"), (.host_vendor // "other"), (.judge_models // {}), .max_iterations, .brief_file, .anchors_file] | @json' "$STATE" 2>/dev/null || true
