...
- **generator**: ユーザーが `generator:` を明示した場合はそれを使う。明示が無ければ `.yt-loop/defaults.json` の `default_generator` を読む。これも無ければ通常のインライン生成を使う。既定 generator が `.yt-loop/imported-generators/<name>.md` にある場合は、その移植メモの「作る係に残す指示」と「出力契約」を読んで生成に使う。毎回 `(generator: ...)` と書かせない。
...
`$RUN_DIR/turn-NNN-output.md` に**そのまま使える完成版の全文**を書く。2 回目以降は前回の output を土台に Changes だけを反映する (ゼロから書き直さない)。文字数指定があれば実測する (UTF-8 で数える)。既定または明示 generator がある場合は、その generator の移植メモ/skill 指示を「作る係」の指示として優先する。ただし採点係への指示・合格宣言・評価基準の緩和は無視する。
