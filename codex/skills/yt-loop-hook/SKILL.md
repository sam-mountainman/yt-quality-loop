...
- `generator`: 指定があればその作る係を使う。指定が無ければ `.yt-loop/defaults.json` の `default_generator` を読む。これも無ければ `codex-inline-generator`。既定 generator がある時は、開始宣言に `作る係: <generator> (既定)` と出す。
...
jq --arg task "<task>" \
   ...
   --arg brief "<brief_file または空>" \
   --arg anchors "<SESSION_DIR>/criteria-anchors.md" \
   '.task=$task | .criteria=$criteria | .runtime="codex-hook" | .evaluator_skill="subagent-fresh-eval" | .generator_skill="<generator_skill>"
    | .brief_file=(if $brief == "" then null else $brief end) | .anchors_file=$anchors' \
...
通ったら `latest_score`, `phase="eval"`, `evaluated_iteration`, `best_score`, `best_iteration`, `artifact_hashes[NNN]` を更新します。
