...
user-invocable: true
argument-hint: "<作りたいものの説明> [skill: <台本スキル名>] [evaluator: fable] [judges: auto|host|host+fable|host+codex|host+grok] [threshold: 90] [max: 6] [criteria: 軸1,軸2,...]"
allowed-tools: "*"
---
...
| **judges** | auto | **合格時の確認採点**を誰が担うか。`auto` = 検出された外部 CLI (fable=claude / codex / grok) から周回採点と別ベンダーを選んで使う。`host` = 外部を使わず従来のフォーク確認のみ。`host+fable` 等で明示指定。外部 1 体 = min 採用 / 2 体以上 = 下側中央値 (2/3 合意)。周回中の採点係は変わらない — 変わるのは合格の門番だけ |
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/confirm-judges.sh" --detect
```

- 出力された候補 (fable / codex / grok) のうち、**周回採点と同じベンダーを除いたもの**を使う。Claude Code では fable (= claude CLI) は同ベンダーなので除外し、codex / grok を採用する
- ユーザーが `judges: host` を指定したら外部を使わない。`judges: host+grok` 等の明示指定はそれに従う (未検出の名前を指定されたら 1 行で伝えて host に落とす)
- 確定した構成を state に記録する (検出結果も開示用に記録):

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-config "<STATE_FILE>" \
  --judges "host,codex,grok" \
  --judges-detected "<--detect の出力をカンマ区切りで>"
```

外部候補が 1 つも無ければ `--judges "host"` (従来動作。機能不足ではなく現行仕様そのもの)。**judges は指紋対象** — ループ開始後に構成を変えると pass gate が合格を拒否する。
...
**A. judges に外部ジャッジ (fable/codex/grok) がある場合** — 確認採点の席は外部ベンダーが担う:

```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/confirm-judges.sh" "$STATE_FILE"
```

- このスクリプトが各外部 CLI を呼び、`turn-$NNN-eval-confirm-<judge>.json` (+ `.fresh` 証明) または `.failed` (理由) を書く。**出力の JSON を手で書いたり編集したりしない** — fresh 証明が無い確認は pass gate が失敗扱いにする
