...
user-invocable: true
argument-hint: "<作りたいものの説明> [skill: <台本スキル名>] [evaluator: fable] [judges: auto|host|host+claude|host+codex|host+grok] [threshold: 90] [max: 6] [criteria: 軸1,軸2,...]"
allowed-tools: "*"
---

# YT Quality Loop

あなたはオーケストレーターです。計画を自分で書き、assign-yt-generator (作る係) → assign-yt-evaluator 系 (採点する係) の 2 フェーズを制御し、品質基準を満たすまでループします。続行/終了の判定はあなたの仕事ではありません — Stop hook (Node/Bash 制御プレーンの整数比較) が行います。
...
| **evaluator** | 自動選択 | 採点する係の skill 名 (明示指定も可)。`fable` は、採点基準を自動選択したまま Fable MCP を fresh evaluator runtime として使う明示指定 |
| **judges** | auto | **合格時の確認採点**を誰が担うか。`auto` = 検出された外部 CLI (`claude` / `codex` / `grok`) から周回採点と別ベンダーを選ぶ。`host` = 外部を使わず従来の確認のみ。`host+claude` 等で明示指定。外部 1 体 = min 採用 / 2 体以上 = 下側中央値。周回中の採点係は変わらない |
...
**Fable の使い分け**: Plan mode では、Fable MCP が利用可能なら criteria / 動画ブリーフ / 採点アンカー / 実行計画のレビューにデフォルトで使ってよい。Plan mode 以外では、ユーザーが `Fable`, `Fable5`, `フェイブル`, `evaluator: fable` のように明示した時だけ使う。採点で使う時は、上の表で `evaluator_skill` と criteria を固定したまま、state の `evaluator_runtime` を `fable` にする。Fable が開始前に使えない/認証失敗した場合は `evaluator_runtime` を `skill` に戻して通常 evaluator で開始し、fallback を開示する。
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" loop-start "$(pwd)" <max> <threshold> <YT_LOOP_SESSION_ID>
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-config "<STATE_FILE>" \
  --task "<task>" \
  --criteria "<criteria>" \
  --evaluator "<evaluator_skill>" \
  --evaluator-runtime "<skill または fable>" \
  --generator "<generator_skill>" \
  --brief "<brief_file または空>"
```

generator / evaluator が未指定なら state.json のデフォルト値のまま (state-config から省略してよい)。macOS/Linux/WSL では従来の `*.sh` も互換用に残っているが、Windows ネイティブでは `yt-loop.js` を使う。
...
1. ユーザーが `anchors: <パス>` を指定していればそのファイルを使う (前回のアンカー、または `/yt-import-skill ... ものさし化` が翻訳した `.yt-loop/skill-criteria/*-anchors.md` の再利用)。`skill:` 指定の generator に対応する `.yt-loop/skill-criteria/<同名>-anchors.md` が存在するのに anchors 未指定なら、1 行だけ案内する (勝手には使わない): 「このスキルの翻訳済みものさしがあります — anchors: <パス> で適用できます」
...
3. `node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-config "<STATE_FILE>" --anchors "<アンカーのパス>"`
...
**確認ジャッジ (judges) の確定**: 合格時の確認採点を誰が担うかを、指紋の記録前に確定する:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/confirm-judges.js" --configure "<STATE_FILE>" \
  --selection "<auto またはユーザー指定>" \
  --host-vendor claude
```

このコマンドがCLI候補を検出し、`auto` ではClaude系を同一ベンダーとして除外し、`judges`・`judges_detected`・`judge_models` をstateへ記録する。Claudeジャッジの既定モデルは `fable`。Codexはconfig、Grokはモデル一覧から設定済みモデルを解決し、解決不能なら `configured-unpinned` と開示する。上書きする場合は開始前に `YT_JUDGE_CODEX_MODEL` / `YT_JUDGE_GROK_MODEL` を設定する。`auto` の外部候補がゼロなら `host`。明示指定した不在CLIは構成から消さず、`.failed` と最終報告に残す。**judges とモデル設定は指紋対象**で、ループ中に変えると合格を拒否する。

**ブリーフ・アンカー・judges を書き終えてから**、最後に指紋を記録する:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" fingerprint "<STATE_FILE>" --record
```

指紋は threshold・criteria・プロファイル・機械ルール・**ブリーフ・アンカー・採点係の定義・judges構成とモデル設定**のハッシュで、ループ中にものさしが変わると Stop hook が合格を拒否する。これは同じローカル権限内の改ざんを不可能にするセキュリティ境界ではなく、通常運用でのすり替えを検知するトリップワイヤ。record は一度しかできないので、必ずこの順序で実行すること。
...
> {外部ジャッジを使う場合: 確認ジャッジ: <judges> — 合格時の確認採点は別ベンダーが担当します (外部を使わない場合は judges: host)}
...
```bash
STATE_FILE="<STATE_FILE>"
TURNS_DIR="$(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" turns_dir)"
ITER="$(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" iteration)"
NNN=$(printf '%03d' $ITER)
ARTIFACT_FILE="$TURNS_DIR/turn-$NNN-output.md"
PROJECT_DIR="$(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" project_dir)"
RULES="$PROJECT_DIR/.yt-loop/mechanical-checks.json"
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" check-mechanical "$ARTIFACT_FILE" "$RULES" && echo "MECH:OK" || echo "MECH:NG"
```
...
ITER="$(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" iteration)"
NNN=$(printf '%03d' $ITER)
# EVAL_FILE には、本文に示した mechanical:true の JSON を Write で保存する。
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-mechanical-ng "$STATE_FILE" "$ITER"
```
...
EVAL_RUNTIME=$(jq -r '.evaluator_runtime // "skill"' "$STATE_FILE")
...
jq -n \
  ...
  --arg evaluator_runtime "$EVAL_RUNTIME" \
  --arg profile_file "$PROFILE_FILE" \
  --arg brief_file "$BRIEF_FILE" \
  --arg anchors_file "$ANCHORS_FILE" \
  '{task:$task, criteria:$criteria, artifact_file:$artifact_file, eval_file:$eval_file, evaluator_skill:$evaluator_skill, evaluator_runtime:$evaluator_runtime, key_instruction:$key_instruction, profile_file:(if $profile_file == "" then null else $profile_file end), brief_file:(if $brief_file == "" or $brief_file == "null" then null else $brief_file end), anchors_file:(if $anchors_file == "" or $anchors_file == "null" then null else $anchors_file end)}' \
  > "$SESSION_DIR/evaluator-context.json"
echo "SKILL:$EVAL_SKILL"
echo "RUNTIME:$EVAL_RUNTIME"
...
- `RUNTIME` 行が `skill` なら、Skill ツールで evaluator を起動する。skill = SKILL 行の値、args = CONTEXT 行のパス。
- `RUNTIME` 行が `fable` なら、Fable MCP の `fable_ask` / `fable_review` 相当のツールを fresh evaluator として使う。Fable には evaluator-context.json の中身と eval JSON 契約だけを渡し、`threshold`、iteration 番号、過去 score、過去 feedback、合格/不合格の期待は渡さない。Fable の返答を編集せず `$EVAL_FILE` に保存し、eval JSON の `evaluator_skill` は SKILL 行の値と一致させる。valid な JSON なら `node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" mark-fresh "$TURNS_DIR" "$NNN"` 相当を実行する。INVALID の場合は Fable を 1 回だけ再実行する。ループ開始後は `evaluator_runtime` を途中変更しない。
...
NNN=$(printf '%03d' $ITER)
EVAL_FILE="$TURNS_DIR/turn-$NNN-eval.json"
THRESHOLD="$(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" threshold)"
CRITERIA_VAL="$(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" criteria)"
SCHEMA_FILE="<3c の SCHEMA 行の値>"

if node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" validate-eval "$EVAL_FILE" "$SCHEMA_FILE" "$THRESHOLD" "$CRITERIA_VAL"; then
  SCORE="$(node -e "console.log(JSON.parse(require('fs').readFileSync(process.argv[1],'utf8')).score)" "$EVAL_FILE")"
...
- **INVALID の場合**: evaluator を同じコンテキストでもう 1 回だけ起動し、再検証する。2 回目も INVALID なら `node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-eval-result "$STATE_FILE" "$ITER" null` で state を更新して応答を終了 (Stop hook の修復パスに任せる)。
- **スコアを自分で決めない・書き換えない。** validate を通った eval JSON の値だけを使う。

### 3d-2. 確認採点 (SCORE >= THRESHOLD の時のみ)

LLM 採点は同一成果物でも±数点ブレる。6 周引けば 1 回はまぐれの 90+ が出る (ガチャ合格)。**合格が出た時は必ず**、独立した確認採点を取る。これは省略できない — **Stop hook の pass gate が「確認採点の実在・本採点とのバイト相違・集計規則 >= threshold」を機械検証し、無い/コピーの合格主張は拒否する**。

**A. judges に外部ジャッジ (claude/codex/grok) がある場合** — 確認採点の席は外部ベンダーが担う:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/confirm-judges.js" "$STATE_FILE"
```

- このスクリプトが各外部 CLI をツール無効で呼び、`turn-$NNN-eval-confirm-<judge>.json` (+ 内容ハッシュの整合マーカー) または `.failed` を書く。**出力JSONを手で書いたり編集したりしない**。整合マーカーはランナー経由の一貫性確認であり、ベンダー署名や暗号学的な出所証明ではない
- 出力の最終行が `RESULT:OK` なら、host のフォーク確認 (下記 B) は不要。そのまま 3e へ
- `RESULT:ALL_FAILED` (外部全滅) なら、**B のフォーク確認も実行してから** 3e へ (降格は最終報告で自動開示される)
- 判定規則は機械側: 外部 1 体 = min(本採点, 外部) >= threshold / 2 体以上 = 下側中央値 >= threshold。採用スコアは常に下げる方向にのみ動く

**B. judges が host のみ (または外部全滅) の場合** — 従来のフォーク確認:

1. 3c と同じコンテキスト JSON の `eval_file` だけを `$TURNS_DIR/turn-$NNN-eval-confirm.json` に変えて、evaluator をもう一度起動する。`evaluator_runtime=fable` の時も別の Fable 呼び出しで確認採点を取る (本採点のコピーで代用しない — バイト一致は機械的に弾かれる)
2. confirm 側も validate-eval.sh で検証する (INVALID なら evaluator を再実行して confirm を取り直す)
3. **confirm の score が低ければ** 差し替えず、state の latest_score に低い方を書けばよい (pass gate が両ファイルを読んで min を採る)。confirm が threshold 未満なら合格主張せず、低い方を latest_score にして通常どおり応答を終了する (ループ続行)
...
```bash
SCORE="$(node -e "console.log(JSON.parse(require('fs').readFileSync(process.argv[1],'utf8')).score)" "$EVAL_FILE")"
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-eval-result "$STATE_FILE" "$ITER" "$SCORE" --artifact "$TURNS_DIR/turn-$NNN-output.md"
echo "Best: iter $(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" best_iteration) ($(node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" state-get "$STATE_FILE" best_score))"
```
...
node "${CLAUDE_PLUGIN_ROOT}/scripts/yt-loop.js" final-report "$STATE_FILE"
```
...
1. **Generator / 通常 Evaluator は Skill ツールで起動する** (Agent ツールではない)。`evaluator_runtime=fable` の時だけ Fable MCP ツールを fresh evaluator として使う。
...
5. **採点軸・threshold・evaluator・judges・プロファイル・機械ルールをループの途中で変えない。** 点が伸びない時に直すのは成果物か計画であって、ものさしではない (グッドハートの法則)。「外部ジャッジが辛いから外す」も同じ抜け道で、変えると指紋照合で合格が拒否される。ものさし自体が壊れていると判断した場合はループを止めてユーザーに相談する。
